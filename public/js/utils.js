var DPRNG = function(initialState) {
    if (initialState === undefined)
    {
        initialState = Math.round(Math.random() * 0xFFFFFFF);
    }

    var counter = 0,
        state = initialState,
        advance = function() {
            var hash = DPRNG.hash(state ^ counter);
        
            state ^= DPRNG.hash(state);

            counter++;
            if (counter > 0xFFFFFFF)
            {
                counter = 0;
            }

            return hash;
        };

    this.nextInt = function(min, max) {
        if (max < min)
        {
            return this.nextInt(max, min);
        }
        else if (max == min)
        {
            return min;
        }

        var rangeSize = max - min,
            nRequiredBits = Math.min(Math.ceil(Math.log(rangeSize) / Math.log(2)), 32),
            result = null;

        if (nRequiredBits > 28)
        {
            var additionalBits = 32 - nRequiredBits,
                mask = Math.pow(2, additionalBits) - 1;
            
            var result = ((advance() << additionalBits) | (advance() & mask));
        }
        else
        {
            var mask = Math.pow(2, nRequiredBits) - 1;
            
            result = (advance() & mask);
        }
        
        // for ranges that are not a 2-complement, larger values than $max may
        // be in $result => reduce to the range by dividing by 2
        while (min + result > max) result = Math.floor(result / 2);
        
        return result;
    };
};
DPRNG.hash = function(_i) {
    var hash = _i;

    // 5 rounds of each: diffusion through substitution, recombine,
    // diffusion through multiplaction and modulo
    for (var i = 0;i < 5;i++)
    {	
        // console.log("round " + i);
        // extract the rightmost 4 bytes
        var byte4 = hash & 0x0F,
            byte3 = (hash >> 4) & 0xFF,
            byte2 = (hash >> 12) & 0xFF,
            byte1 = (hash >> 20) & 0xFF;

        // substitute and recombine
        hash = (DPRNG.hash.diffusionTable[byte1] << 20) |
               (DPRNG.hash.diffusionTable[byte2] << 12) |
               (DPRNG.hash.diffusionTable[byte3] << 4 ) |
               byte4;

        // multiplicate and reduce to 24 bit range
        hash = Math.abs(hash * 0x7) % 0xFFFFFFF;
    }

    return hash;
};
DPRNG.hash.diffusionTable = [
    
];

// -----------------------------------------------------------------------------

var TransactionQueue = function() {
    var items = [],
        currentTransactionPromise = null,
        queuedDuringTransaction = [],
        _self = this;
    
    this.pushItem = function(item) {
        if (currentTransactionPromise != null)
        {
            queuedDuringTransaction.push(item);
        }
        else
        {
            items.push(item);
        }
    };
    
    this.consume = function(nItems, callback) {
        if (currentTransactionPromise != null)
        {
            return ~function(nItems, callback) {
                return currentTransactionPromise.then(function() {
                    return _self.consume(nItems, callback);
                });
            }(nItems, callback);
        }
        else
        {
            ~function(transactionItems, callback) {
                currentTransactionPromise = new Promise(function(resolve, reject) {
                    if (transactionItems.length == 0)
                        resolve();
                    
                    callback(transactionItems, resolve, reject);
                });
                
                currentTransactionPromise.then(function() {
                    if (queuedDuringTransaction.length != 0)
                    {
                        items = items.concat(queuedDuringTransaction);
                        queuedDuringTransaction = [];
                    }
                    
                    currentTransactionPromise = null;
                }, function() {
                    // put things back into order
                    items = items.concat(transactionItems).concat(queuedDuringTransaction);
                    queuedDuringTransaction = [];
                    
                    currentTransactionPromise = null;
                });
            }(
                items.splice(0, Math.min(items.length, nItems)),
                callback
            );
    
            return currentTransactionPromise;
        }
    };
    
    this.hasItems = function() {
        if (currentTransactionPromise != null)
        {
            return true;
        }
        else
        {
            return items.length > 0;
        }
    };
};